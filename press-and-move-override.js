...
const EPS = 10e-6;
/**
 * Rotate camera with mouse movement; User camera movement with the mouse.
...
AFRAME.registerComponent('press-and-move', {
    ...
    init() {
        ...
        // Remove the touchstart and touchend event listeners

        window.addEventListener('mousemove', (evt) => {
            // Adjust the camera rotation based on mouse movement
            self.el.object3D.rotation.y -= evt.movementX * 0.002;
            self.el.object3D.rotation.x -= evt.movementY * 0.002;
            // Clamp the x rotation to prevent flipping
            self.el.object3D.rotation.x = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, self.el.object3D.rotation.x));
        });
    },
